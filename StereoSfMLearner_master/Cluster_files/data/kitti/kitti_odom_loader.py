...
class kitti_odom_loader(object):
    def __init__(self,
        ...
        self.train_seqs = [0, 1, 2, 3, 4, 5, 6, 7, 8, 11, 12, 13, 14, 15, 16, 17, 18, 19]
        self.test_seqs = [9, 10]
    ...
    # argument added: camera ('2' or '3': string)
    def load_image_sequence(self, frames, tgt_idx, seq_length, camera):
        half_offset = int((seq_length - 1)/2)
        image_seq = []
        for o in range(-half_offset, half_offset+1):
            curr_idx = tgt_idx + o
            curr_drive, curr_frame_id = frames[curr_idx].split(' ')
            curr_img = self.load_image(curr_drive, curr_frame_id, camera)
            if o == 0:
    ...
    # argument added: camera ('2' or '3': string)
    def load_example(self, frames, tgt_idx, camera, load_pose=False):
        image_seq, zoom_x, zoom_y = self.load_image_sequence(frames, tgt_idx, self.seq_length, camera)
    ...
    # argument added: camera ('2' or '3': string)
    def get_train_example_with_idx(self, tgt_idx, camera):
        if not self.is_valid_sample(self.train_frames, tgt_idx):
            return False
        example = self.load_example(self.train_frames, tgt_idx, camera)
        return example
    ...
    # argument added: camera ('2' or '3': string)
    def load_image(self, drive, frame_id, camera):
        img_file = os.path.join(self.dataset_dir, 'sequences', '%s/image_%s/%s.png' % (drive, camera, frame_id))
